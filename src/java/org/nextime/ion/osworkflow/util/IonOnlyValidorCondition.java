 ...
 * @author gbort
 */
public class IonOnlyValidorCondition implements Condition {
	public boolean passesCondition(Map inputs, Map args, PropertySet variables) {
		try {
			...
			return User.getInstance(context.getCaller()).isInGroup(Group.getInstance(args.get("group") + ""))
					&& User.getInstance(context.getCaller()).isInGroup(
							Group.getInstance("gValideurs"));
		} catch (Exception e) {
			// TODO
			Logger.getInstance().error(e.getMessage(), IonOnlyValidorCondition.class,e);
			return false;
		}
	}
}