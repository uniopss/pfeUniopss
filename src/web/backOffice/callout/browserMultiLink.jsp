...
<html>
<head>
        <title>Traitement <c:out value="${param['btype']}"/></title>
...
function lcC() {
        ...
        for(var x=0;x<callout.document.FileList.elements.length;x++){
                ...
                if (y.checked) {
                        ...
                        window.opener.document.frm.<c:out value="${param['action']}"/>.value=id;
                        break;
                }
	}
	<c:choose>
        <c:when test="${param['btype']=='_lien'}">
        eval(window.opener.document.frm.<c:out value="${param['action']}"/>(id));
        </c:when>
   </c:choose>
        self.close();
}
</script>
<form id=ress name=ress action="/backoffice/callout/browserMultiLink.jsp" >
	...
	<table width="100%">
		...
		<td align="left" class="text">
			...
			Type&nbsp;
			<select name="selectedType" style="width=150" onChange="document.ress.submit()">
...
<iframe id="callout" name="callout" src="/backoffice/edx/browserMultiLink.jsp?action=<c:out value="${param['action']}"/>&selectedSite=<c:out value="${selectedSite}"/>&selectedType=<c:out value="${selectedType}"/>"
